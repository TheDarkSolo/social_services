...
function t778__showMore(recid) {
    var rec = document.querySelector('#rec' + recid + ' .t778'); if (!rec) return; var cardsShowLimit = parseInt(rec.getAttribute('data-show-count'), 10); if (isNaN(cardsShowLimit) || cardsShowLimit <= 0) return; var showMoreButton = rec.querySelector('.t778__showmore'); if (!showMoreButton) return; if (!showMoreButton.querySelector('td').textContent) { showMoreButton.querySelector('td').textContent = t778__dict() }
    ...
    showMoreButton.addEventListener('click', function () {
        var currentColumns = rec.querySelectorAll('.t778__col'); var currentColumnsShowed = 0; Array.prototype.forEach.call(currentColumns, function (column) { if (column.style.display === 'inline-block') { ++currentColumnsShowed } }); for (var i = 0; i < cardsShowLimit + currentColumnsShowed; i++) { if (allProductCards[i]) { allProductCards[i].style.display = 'inline-block' } }
...
function t778__showSeparator(rec, cardsShowLimit) {
    if (window.jQuery && rec instanceof jQuery) { rec = rec.get(0) }
...
function t778__hoverZoom_init(recid) {
    if (window.isMobile) return; var rec = document.getElementById('rec' + recid); if (!rec) return; try {
        if (!rec.querySelector('[data-hover-zoom]')) { return }
...
function t778__alignButtons_init(recid) {
    var rec = document.getElementById('rec' + recid); if (!rec) return; if (!rec.querySelector('[data-buttons-v-align]')) { return }
    try {
        t778__alignButtons(recid); if (document.querySelector('.t-records').getAttribute('data-tilda-mode') === 'edit') { setInterval(function () { t778__alignButtons(recid) }, 200) }
        window.addEventListener('resize', t_throttle(function () {
            if (window.noAdaptive && window.noAdaptive && window.isMobile) { return }
            t778__alignButtons(recid)
        })); var wrapperBlock = rec.querySelector('.t778'); if (wrapperBlock) { wrapperBlock.addEventListener('displayChanged', function () { t778__alignButtons(recid) }) }
        if (window.isMobile) { window.addEventListener('orientationchange', function () { t778__alignButtons(recid) }) }
    } catch (error) { console.log('buttons-v-align error: ' + error.message) }
}
function t778__alignButtons(recid) {
    var rec = document.getElementById('rec' + recid); if (!rec) return; var contents = rec.querySelectorAll('.t778__content'); var maxHeight = 0; var maxHeightBtns = 0; var container = rec.querySelector('.t-container'); if (container) { var itemsInRow = parseInt(container.getAttribute('data-blocks-per-row'), 10) }
    ...
    var count = 1; var textWrappersInRow = []; var btnWrappersInRow = []; Array.prototype.forEach.call(contents, function (content) {
        var textWrapper = content.querySelector('.t778__textwrapper'); if (textWrapper) {
            textWrapper.style.height = 'auto'; if (itemsInRow === 1) { textWrapper.style.height = 'auto' } else {
                textWrappersInRow.push(textWrapper); if (textWrapper.offsetHeight > maxHeight) { maxHeight = textWrapper.offsetHeight }
                if (maxHeight > 0) { Array.prototype.forEach.call(textWrappersInRow, function (textWrapper) { textWrapper.style.height = maxHeight + 'px' }) }
            }
        }
        var btnWrapper = content.querySelector('.t778__btn-wrapper'); if (btnWrapper) {
            btnWrapper.style.marginTop = ''; if (itemsInRow === 1) { btnWrapper.style.marginTop = '' } else {
                btnWrappersInRow.push(btnWrapper); if (btnWrapper.offsetHeight > maxHeightBtns) { maxHeightBtns = btnWrapper.offsetHeight }
...
function t778_initPopup(recid) {
    var rec = document.getElementById('rec' + recid); if (!rec) return; var allPopupLinks = rec.querySelectorAll('[href^="#prodpopup"]'); if (!allPopupLinks.length) return; var popup = rec.querySelector('.t-popup'); popup.addEventListener('mousedown', function (event) {
        var windowWidth = window.innerWidth; var maxScrollBarWidth = 17; var windowWithoutScrollBar = windowWidth - maxScrollBarWidth; if (event.clientX > windowWithoutScrollBar) { return }
        if (event.target === this) { t778_closePopup(document.body, popup) }
    }); Array.prototype.forEach.call(allPopupLinks, function (popupLink) {
        var product = popupLink.closest('.js-product'); var productLid = product.getAttribute('data-product-lid'); var productLinks = document.querySelectorAll('.r a[href$="#!/tproduct/' + recid + '-' + productLid + '"]'); Array.prototype.forEach.call(productLinks, function (productLink) { productLink.addEventListener('click', function () { if (rec.querySelector('[data-product-lid="' + productLid + '"]')) { var linkToPopup = product.querySelector('[href^="#prodpopup"]'); if (linkToPopup) { var event = document.createEvent('HTMLEvents'); event.initEvent('click', !0, !1); linkToPopup.dispatchEvent(event) } } }) }); popupLink.addEventListener('click', clickOnceHandler, { once: !0 }); function clickOnceHandler(event) { event.preventDefault(); t_onFuncLoad('t_sldsInit', function () { t_sldsInit(recid + ' #t778__product-' + productLid) }); popupLink.removeEventListener('click', clickOnceHandler, !1) }
        popupLink.addEventListener('click', function (event) {
            event.preventDefault(); if (event.target.classList.contains('t1002__addBtn') || event.target.parentNode.classList.contains('t1002__addBtn')) { return }
            ...
            var analitics = popup.getAttribute('data-track-popup'); if (analitics && productFull && window.Tilda) {
                var productName = productFull.querySelector('.js-product-name'); if (productName) {
                    var virtualTitle = productName.textContent; if (!virtualTitle) { virtualTitle = 'prod' + productLid }
            ...
            t778_updateSlider(recid + ' #t778__product-' + productLid); setTimeout(function () { if (window.lazy === 'y' || document.getElementById('allrecords').getAttribute('data-tilda-lazy') === 'yes') { t_onFuncLoad('t_lazyload_update', function () { t_lazyload_update() }) } }, 500)
        })
    }); if (!document.getElementById('record' + recid)) { t778_checkUrl(recid) }
}
function t778_checkUrl(recid) {
    var currentUrl = window.location.href; var tprodIndex = (currentUrl.indexOf('#!/tproduct/') + 1 || currentUrl.indexOf('%23!/tproduct/') + 1 || currentUrl.indexOf('#%21%2Ftproduct%2F') + 1 || currentUrl.indexOf('#!%2Ftproduct%2F') + 1 || currentUrl.indexOf('%23%21%2Ftproduct%2F') + 1) - 1; if (tprodIndex !== -1) {
        var currentUrl = currentUrl.substring(tprodIndex, currentUrl.length); var curProdLid = currentUrl.substring(currentUrl.indexOf('-') + 1, currentUrl.length); if (curProdLid) { var curProdLidMatch = curProdLid.match(/([0-9]+)/g); if (curProdLidMatch) { curProdLid = curProdLidMatch[0] } }
...
function t778__unifyHeights(recid) {
    ...
    for (var i = 0; i < cards.length; i += perRow) {
        var maxHeight = 0; for (var j = 0; j < perRow; j++) {
            if (cards[i + j]) {
                var card = cards[i + j].querySelector('.t778__content'); var txt = card.querySelector('.t778__textwrapper'); var btns = card.querySelector('.t778__btn-wrapper'); var height = 0; if (txt) { height += txt.offsetHeight }
